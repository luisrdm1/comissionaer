...
def compile_block(code: str, preamble: bool = True, keep: bool = False) -> tuple[bool, str]:
    """Try to compile a Typst code block. Returns (success, stderr)."""
    full_code = (PREAMBLE + "\n" + code) if preamble else code

    with tempfile.NamedTemporaryFile(suffix=".typ", mode="w", delete=not keep, dir=".") as f:
        f.write(full_code)
        f.flush()
        tmp_path = f.name
