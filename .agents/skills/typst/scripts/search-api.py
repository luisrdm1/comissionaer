...
def bm25_search(query_tokens, bm25, top_n=20):
    ...
    for token in query_tokens:
        ...
        for doc_id_str, tf in bm25["postings"].get(token, []):
            ...
            dl = bm25["doc_lengths"].get(str(doc_id), bm25["doc_lengths"].get(doc_id, avg_dl))
...
def format_entry(entry, verbose=False):
    ...
    if verbose:
        ...
        for p in entry.get("params", []):
            if p.get("strings"):
                lines.append(f"  {p['name']} values: {', '.join(repr(s) for s in p['strings'])}")
    return "\n".join(lines)
...
def main():
    ...
    parser.add_argument("--category", help="Filter by category (e.g., Foundations, Layout)")
    ...
    if args.category:
        candidate_ids = {
            i for i in candidate_ids if args.category.lower() in api[i]["category"].lower()
        }
