...
def aggregate(records, self_times=None):
    ...
    agg = defaultdict(lambda: {"count": 0, "total": 0, "self": 0, "max": 0, "sources": set()})
    for r in records:
        name = r["name"]
        dur = r["dur"]
        a = agg[name]
...
def sort_key(sort_by, use_self=False):
    ...
    if sort_by == "name":
        return lambda item: (item[0] if isinstance(item[0], str) else item[0][0]).lower()
...
def main():
    ...
    wall_us = max(r["start"] + r["dur"] for r in records) - min(r["start"] for r in records)
    if not args.json_output:
        print(f"Trace: {len(records)} events, {len(tids)} threads, {us_to_ms(wall_us)}ms wall time")
        print()
    ...
    items = sorted(items, key=sort_key(args.sort, use_self), reverse=args.sort != "name")
    ...
    if use_self:
        hdr = f"{name_hdr} {'Count':>7} {'Total':>10} {'Self':>10} {'Avg':>9} {'Max':>9}"
    else:
